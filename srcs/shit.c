...
char	*ft_malloc_zero_flag(t_data *tab, char *a)
{
	...
	if (!anew)
		return (NULL);
	anew = ft_add_zeros_flag(tab, a, anew);
	free(a);
	return (anew);
}
...
void	ft_print_d(t_data *tab)
{
	...
	if (a[0] == '0' && tab->point && tab->precision == 0)
		a[0] = '\0';
	else if (tab->precision >= ft_strlen(a))
		a = ft_malloc_for_zeros(tab, a);
	else if (tab->zero && tab->width > ft_strlen(a) && !tab->dash && !tab->point)
...
void	ft_print_p(t_data *tab)
{
	...
	a = ft_ulitoa_hex(va_arg(tab->args, unsigned long long));
	if (a[0] == '0')
	{
        ft_print_nil(tab);
        return ;
...
int main() {
  ...
  unsigned int u = 600000;
  char *str = "hi there charlie";
  char c = 'c';
  int x = 12345;
  int *p = &x;
  //unsigned int y = 65465654;
  len1 = ft_printf("||%5c %.5s %09.4d %08u %-25.20p||", c, str, x, u, p);
  ft_printf("\n");
  len2 = printf("||%5c %.5s %09.4d %08u %-25.20p||", c, str, x, u, p);
